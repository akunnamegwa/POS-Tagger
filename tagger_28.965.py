...
def observation_prob(tag, word):
    ...
    """
    observations = tag_bank[tag]  # count of all words with tag
...
# There are two versions of the word inference function. They differ slightly.

def word_inference1(word, prev_tag=None):
    # make observations 1 if the word is unseen
    # change it so that observed words with low transitions are ignored
    if word in words:
    ...
    # make transition probabilities equal to initial probabilies if it is the first word
    if prev_tag is None:
        diff = len(trans_matrix[0]) - len(initial_probabilities)
        diff_array = np.ones(diff)
        prev_tag_transitions = np.concatenate((initial_probabilities, diff_array))
    ...
    # account for observations with 0 trans
...
def fine_tune(sentences):
    """
    :type sentences: list[tuple[str, str]]
    :return: This function changes the results based on logical conclusion of English structure.
    """
    for i in range(len(sentences)):
        ...
        if word != '':
            if tag == 'NN1' and word[-1] == 's' and word not in words:
